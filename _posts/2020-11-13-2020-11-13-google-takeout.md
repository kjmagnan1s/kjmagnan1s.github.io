---
layout: post
title: "Google Takeout Data Analysis"
date: 2020-11-13
author: Kevin Magnan
categories: [Data Analysis, Personal]
description: "Analysis of personal Google Takeout data"
thumbnail: /assets/images/blog/2020-11-13-google-takeout/thumbnail.png
---
...
## read initial datamysearchfile <- read_html("~/GitHub/blog_website_2.0/blog_data/google_takeout/Takeout/My Activity/Search/MyActivity.html", encoding = "UTF-8")
...
## Scraping search date and timedateSearched <- mysearchfile %>% 
...
## Scraping search texttextSearch <- mysearchfile %>% 
...
## Scraping search typetypeSearch <- mysearchfile %>% 
...
[$ search    <chr> "https://www.businessinsider.com/number-one-observatory-circ~]($ search    <chr> "https://www.businessinsider.com/number-one-observatory-circ~)$ type      <chr> "Visited", "Visited", "Searched", "Searched", "Searched", "S~
...
## searches over timesearchedData %>% 
...
![](/assets/images/blog/2020-11-13-google-takeout/searches%20over%20time-1.png)
...
![](/assets/images/blog/2020-11-13-google-takeout/search%20by%20hour-1.png)
...
![](/assets/images/blog/2020-11-13-google-takeout/search%20by%20day-1.png)
...
## text mine google search phrasestextData <- searchedData %>% 
...
## side by side wordcloudspar(mfrow=c(1,2))



## wordcloud for search termstextCloud <- wordcloud(words = textDT$word, main = "Title", 
...
![](/assets/images/blog/2020-11-13-google-takeout/text%20mining%20searches-1.png)
...
![](/assets/images/blog/2020-11-13-google-takeout/text%20mining%20searches%202017-1.png)
...
## plot search terms over time, by termggplotly(
...
## plot search terms for 2017ggplotly(
...
![](/assets/images/blog/2020-11-13-google-takeout/giphy.gif)
...
## Not ran - time intensive process to load, manipulate and write 2.2gb json file of location history.## I suggest running this once with the cleaning steps and writing to another file, as I have done below

# # load google map location data# loc_hist_takeout <- fromJSON("~/GitHub/kjmagnan1s.github.io/blog_data/google_takeout/Takeout/My Activity/Maps/Location History.json", flatten = TRUE)# locations <- loc_hist_takeout$locations# rm(loc_hist_takeout) ## free up memory from large json files# # ## clean json data# locations <- locations %>% #   ## convert the time column from milliseconds#   mutate(time = as.POSIXct(as.numeric(timestampMs)/1000, origin = "1970-01-01")) %>%#   ## clean/arrange date columns#   mutate(day = day(time)) %>% #   mutate(day = ordered(day, levels = c("Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"))) %>% #   ## conver lat/long from E7 to coordinates#   mutate(lat = latitudeE7/1e7) %>% #   mutate(long = longitudeE7/1e7) %>% #   ## remove unnecessary columns#   select(time, lat, long)# # ## write to csv for later use, maybe a Tableau dashboard# write.csv(locations, file = "~/GitHub/kjmagnan1s.github.io/blog_data/google_takeout_location_data.csv")

## load pre-cleaned location history datalocations <- read.csv("~/GitHub/blog_website_2.0/blog_data/google_takeout_location_data.csv")
...
# ## load map searchesmymapfile <- read_html("~/GitHub/blog_website_2.0/blog_data/google_takeout/Takeout/My Activity/Maps/MyActivity.html", encoding = "UTF-8")



## re-running the cleaning code on the google map searches## Scraping map date and timedateMapped <- mymapfile %>% 
...
## Scraping map texttextMapped <- mymapfile %>% 
...
## Scraping map typetypeMapped <- mymapfile %>% 
...
## map searches over timemappedData %>% 
...
![](/assets/images/blog/2020-11-13-google-takeout/map%20searches%20over%20time-1.png)
...
## map views/searches by yearggplotly(
...
## map activity over time, heatmapmappedData %>%
...
## map my location history data for Chicagoregister_google(Sys.getenv("my_api"), ## Your API key goes here!
...
![](/assets/images/blog/2020-11-13-google-takeout/location%20history%20map-1.png)
...
# map my specific location for December 2018locations %>%
