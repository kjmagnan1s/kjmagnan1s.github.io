...
ggplotly(
  ...
  summarise(EP_Decay = sum(Value[TransactionType %in% c("EpDecay")]),
            GP_Decay = sum(Value[TransactionType %in% c("GpDecay")])) %>%
  gather(Decay, Value, -CharacterName) %>% 
  #slice_max(order_by = Value, n = 20) %>%
  ggplot(aes(x = fct_reorder(CharacterName, Value, .desc = TRUE), y = Value, fill = Decay, text = paste(
    CharacterName, "has accrued", Value, "GP Decay"))) +
  geom_col(position = "dodge") +
  theme(axis.text.x = element_text(angle = 75)) +
    labs(
    title = "Decay Values",
...
## ggplot decay values across users
```
...
ggplotly (
  ...
  summarise(Rolls = sum(Type == "Roll"),
            Items = n()) %>% 
  ggplot() + 
  geom_col(aes(x = TransactionDate, y = Items, text = paste(Items, "items were claimed", TransactionDate))) +
