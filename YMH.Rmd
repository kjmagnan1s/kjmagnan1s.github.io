...
playlist_clean$url <- gsub("s#", "s:", playlist_clean$url)
...
playlist_clean <- playlist_clean %>% 
  ...
  mutate(vid = str_replace_all(url, ".*=(.*)$", "\\1"))    # creates a vid column of just the youtube short link
head(playlist_clean)
```
