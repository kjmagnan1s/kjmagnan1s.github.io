...
First thing you'll want to do is download the youtube-dl .exe windows application from their site and place it into a folder. Open a cmd window and navigate to that folder directory. Lucky for you, I've done all the troubleshooting and testing of this tool. These are the commands I decided to use, mainly due to my limited knowledge of the windows command line. 
...
You should end up with two .csv's with a single column of data. Let's take a look and quickly clean and cbind them:
...
You didn't think it would be that easy, did you? Turns out the command-line export creates a long data set and not a wide data set. Let's fix that before we continue and then do some additional cleaning.
...
```{r}
# filtering out the episode titles and episode url id's into separate df's
playlist_url <- playlist_raw %>% filter(!(grepl("Your", V1))) %>% rename("url" = V1) %>% filter(!(grepl(" ", url)))
playlist_title <- playlist_raw %>% filter(grepl("Your", V1)) %>% rename("title" = V1)
playlist_clean <- cbind(playlist_title, url = playlist_url$url[1:245])
head(playlist_clean)
## paste("https://www.youtube.com/watch?v=",url, sep = "")
...
Success! We have 245 rows of observations of YMH episodes, #330 to #572, with titles and URLs. Now we can start working on pulling the transcripts for all 240 episodes.
