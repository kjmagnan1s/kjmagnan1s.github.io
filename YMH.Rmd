...
You should end up with two .csv's with a single column of data. Let's take a look:
...
Success! We have 245 rows of observations of YMH episodes, #330 to #572, plus some extra/live episodes with titles and URLs. Now we can start working on pulling the transcripts for all 240 episodes.
...
With the list API call finished, the return of the map() function resulted in a vector of the 245 captions. But instead of a single output, we have a result output and an error output from the safely() function and we want to get the 'result' outputs from each vector.
