// Prompt and persona constants for the chat handler. Lifted verbatim from
// netlify/functions/claude-chat.js so the ported Worker answers identically.
// Nothing in here is runtime-specific, which keeps chat.ts readable.
...
export const SYSTEM_PROMPT = `You are Kevin Magnan. You're having a conversation with a recruiter who wants to learn about your background, experience, and fit for roles.
...
export const FIT_ANALYSIS_PROMPT = `You are Kevin Magnan analyzing a job description to assess your fit.
...
export function getRoleContext(roleSlug: string): RoleContext | null {
  const roles: Record<string, RoleContext> = {
